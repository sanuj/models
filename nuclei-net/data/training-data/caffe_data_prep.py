...
root = '/home/test/Projects/nuclei-net-data/TCGA-data/'

def saveIm(im, label, h, i, j, k, f_name):
    path = root + str(f_name) + '/' + str(k) + '/'
...
for f_name in range(2, 17):
	file_name = root + 'training_images/' + str(f_name) + '_norm.bmp'
	mask_name = root + 'training_maps/' + str(f_name) + '.png'
	...
	for i in range(p, height-p, 3):
    		for j in range(p, width-p, 3):
        		if h >= 25000:
            			k = k+1
            			h = 0
        		if not (image[i,j] >= [220, 220, 220]).all():
            			temp_x = image[i-p:i+p+1, j-p:j+p+1, :]
            			saveIm(temp_x, mask[i, j], h, i, j, k, f_name)
            			h = h+1
